...
export async function create() {
  ...
  const mapeo = { id, title: `Mapeo #${id}`, createdAt: now, updatedAt: now, codes: [] };
...
export async function addCode(mapeoId, rawCode) {
...
export async function updateCode(mapeoId, codeId, patch) {
  const mapeo = mapeos.find((x) => x.id === mapeoId);
  if (!mapeo) throw new Error('NOT_FOUND');
...
export async function removeCode(mapeoId, codeId) {
  const mapeo = mapeos.find((x) => x.id === mapeoId);
  if (!mapeo) throw new Error('NOT_FOUND');
