/* ============================================================
   ...
   Abrir un contenedor empuja el de abajo (nunca se superponen) y
   cierra automáticamente cualquier otro que estuviera abierto. Una
   vez validado correctamente, el contenedor queda bloqueado (no se
   puede reabrir ni tiene más lógica) y su avatar pasa de "pendiente" a
   "éxito".

   Un solo <video> de cámara se reutiliza entre los contenedores 1 y 2
   (se monta dentro del que esté abierto) — así, en la futura versión
   handheld (sin cámara), alcanza con no crearlo: el resto de la
   pantalla (avatares, textos, input manual) funciona igual.
...
export function openValidation(item, { onDone }) {
  ...
  overlay.innerHTML = `
    ...
    <div class="scan-sheet cq-sheet venc-acc-body" id="vencAccBody">
      <div class="venc-acc-item" data-state="pending" id="itemCaja">
        ...
        <div class="venc-acc-wrap" id="wrapCaja">
          <div class="venc-acc-inner">
            <div class="venc-acc-panel" id="panelCaja">
      ...
      <div class="venc-acc-item" data-state="locked" id="itemProd">
        ...
        <div class="venc-acc-wrap" id="wrapProd">
          <div class="venc-acc-inner">
            <div class="venc-acc-panel" id="panelProd">
  ...
  const wrapCaja = overlay.querySelector('#wrapCaja');
  ...
  const wrapProd = overlay.querySelector('#wrapProd');
  ...
  function setOpen(key, isOpen) {
    (key === 'caja' ? wrapCaja : wrapProd).classList.toggle('is-open', isOpen);
  }
  ...
  function renderCajaControls() {
    controlsCaja.innerHTML = `
      ${mismatchHTML(mismatch.caja)}
      <form class="scan-manual" id="formCaja">
        <input type="text" inputmode="numeric" placeholder="Ingresar número de caja" autocomplete="off" />
        <button type="submit" class="btn btn-primary" title="Confirmar">${icon('check', 18)}</button>
      </form>
    `;
    controlsCaja.querySelector('#formCaja').addEventListener('submit', (e) => {
      e.preventDefault();
      const input = controlsCaja.querySelector('input');
      const value = input.value.trim();
      if (!value) return;
      checkMatch('caja', value, item.caja);
      input.value = '';
    });
  }
  ...
  function renderProdScanControls() {
    controlsProd.innerHTML = `
      ...
      <form class="scan-manual" id="formProd">
        <input type="text" inputmode="numeric" placeholder="Ingresar código de barras" autocomplete="off" />
        <button type="submit" class="btn btn-primary" title="Confirmar">${icon('check', 18)}</button>
      </form>
      <button type="button" class="venc-val-skip" id="skipProd">No lo encuentro — reportar faltante</button>
    `;
    controlsProd.querySelector('#formProd').addEventListener('submit', (e) => {
      e.preventDefault();
      const input = controlsProd.querySelector('input');
      const value = input.value.trim();
      if (!value) return;
      checkMatch('prod', value, item.referencia);
      input.value = '';
    });
